...
def parse_args():
    parser = argparse.ArgumentParser(description='Assign pose to Neg pkl')
...
args = parse_args()
data = pickle.load(open('Data/Trainval_Neg_HICO_with_pose.pkl', 'rb'), encoding='latin1')
    ...
    if os.path.exists(os.path.join(args.pose, 'HICO_train2015_%08d_keypoints.json' % key)):
        ...
        for i in range(len(f['people'])):
            ...
            for j in range(len(data[key])):
                if data[key][j][7] is not None:
                    alphapose = np.array(data[key][j][7]).reshape(-1, 3)[sel, :2]
                    dis = calc_mse(body_pose, alphapose)
                    if (used[j] < 0 and dis < 500) or (dis < used[j]):
                        used[j] = dis
                        data[key][j][-1] = i
pickle.dump(data, open('Data/Trainval_Neg_HICO_with_idx.pkl', 'wb'), protocol=2)